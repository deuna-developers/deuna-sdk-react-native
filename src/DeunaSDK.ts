...
export class DeunaSDK extends BaseDeuna {
  ...
  async initElements(
    ...
    const walletProvider = firstType
      ? WALLET_TYPE_TO_PROVIDER[firstType.toUpperCase()]
      : undefined;
  ...
  close = async (): Promise<void> => {
    if (!this.deunaWidgetManager.isInitialized) {
      this.externalUrlHelper.resetInAppBrowserReopenGuard();
      return;
    }
        ...
        try {
          ...
          if (
            ...
            !this.externalUrlHelper.canOpenInAppBrowserUrl(url, {
              bypassReopenGuard: options?.userInitiated === true,
            })
          ) {
            return;
          }
