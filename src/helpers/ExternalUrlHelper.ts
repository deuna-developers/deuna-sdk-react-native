...
export class ExternalUrlHelper {
  ...
  canOpenInAppBrowserUrl(
    url: string,
    options?: { bypassReopenGuard?: boolean }
  ): boolean {
    if (this.manager?.type === ExternalUrlBrowser.IN_APP_BROWSER) {
      return false;
    }

    if (options?.bypassReopenGuard) {
      return true;
    }
  ...
  resetInAppBrowserReopenGuard(): void {
    if (this.manager?.type === ExternalUrlBrowser.IN_APP_BROWSER) {
      this.manager = null;
      this.completeClose();
    }
    this.completer = null;
    this.lastInAppBrowserSignature = null;
    this.lastInAppBrowserClosedAt = 0;
  }
